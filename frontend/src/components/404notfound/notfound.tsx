...
//FULL CREDIT TO https://codepen.io/code2rithik/pen/BapYBoP
//Doing it to translate templates into actual app.
...
const GlitchyText: React.FC<GlitchyTextProps> = ({ children, ...props }) => {
    return (
        <h1 {...props} className={`glitchy-text ${props.className}`}>
            ...
            {children.split('').map((char, idx) => {
                const charStyle: CSSProperties & Record<string, string> = {
    ...
    useEffect(() => {
        ...
        const update = (e: any) => {
            if (e.acceleration && e.acceleration.x !== null) {
                root.style.setProperty('--X', e.acceleration.x.toString());
                root.style.setProperty('--Y', e.acceleration.y.toString());
            } else {
